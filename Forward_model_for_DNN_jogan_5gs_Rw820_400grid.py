...
Ds = np.array([[500 * 10**-6], [420 * 10**-6],
               [354 * 10**-6],
               [297 * 10**-6], [250 * 10**-6]])  # representative diameters of grain-size classes
...
read_setfile("config_g5_400grid_gs.ini")
(x, C, spoints, deposit) = forward([4000,2.0,1.5, 0.01, 0.01, 0.01, 0.01,0.01])
#ipdb.set_trace()
...
import numpy as np
icond = np.loadtxt('start_param_random_5500_jogan_gs5_rw820_400grid.csv', delimiter=',')
#icond_new=icond[:,1:]
...
print(icond.shape)
eta = np.loadtxt('eta_5500_g6_300grid_jogan_gs5_rw820_400grid.csv', delimiter=',')
